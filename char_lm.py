"""
Character-Level Input Language Model 
...
from layers import uniform_weight, xavier_weight
from layers import param_init_fullyconnected_layer, fullyconnected_layer
from layers import param_init_lstm_xavier, lstm_layer, char_lstm_layer
...
layers_ = {'fc': ('param_init_fullyconnected_layer', 'fullyconnected_layer'),
           'lstm_x': ('param_init_lstm_xavier', 'lstm_layer'),
...
def init_params(options):
    ...
    params = get_layer('lstm_x')[0](options, params, 'lstmlm_1', options['dim_word'], options['dim_lm_lstm'])
    params = get_layer('lstm_x')[0](options, params, 'lstmlm_2', options['dim_lm_lstm'], options['dim_lm_lstm'])
...
def build_model(tparams, options):
    ...
    # 2-layer LSTM-LM
    proj = get_layer('lstm_x')[1](tparams, Cemb, options, 'lstmlm_1', x_last_chars)
    proj = get_layer('lstm_x')[1](tparams, proj[0], options, 'lstmlm_2', x_last_chars)
    proj_h = proj[0] 
    ...
    return trng, is_train, x_f, x_r, x_spaces, x_last_chars, label_words, cost
...
def perplexity(f_cost, lines, char_dict, word_dict, opts):
    ...
    for _, index in kf_train:
        ...
        cost_one = f_cost(x_f_, x_r_, x_spaces_, x_last_chars_, label_words_) * x_f_.shape[1]
        cost += cost_one
...
def train(opts):
    ...
    trng, is_train, x_f, x_r, x_spaces, x_last_chars, label_words, cost = build_model(tparams, opts)
    inps = [x_f, x_r, x_spaces, x_last_chars, label_words]
    ...
    for eidx in xrange(opts['max_epochs']):
        ...
        for _, index in kf_train:
            ...
            # format input data
            x_f_, x_r_, x_spaces_, x_last_chars_, _, label_words_ \
                                                     = txt_to_inps(x, char_dict, word_dict, opts) 
            
            # compute cost 
            cost = f_grad_shared(x_f_, x_r_, x_spaces_, x_last_chars_, label_words_)     
